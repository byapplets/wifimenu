...
# Script setup
# Theming
conf_dir=~/.config/rofi/config
theme_wifimenu=$conf_dir/wifimenu.rasi
theme_pwd_prompt=$conf_dir/pwd.rasi

# Wifi daemon setup
wifidaemon_dir=~/.config/rofi/scripts/wifidaemon
wifidaemon_bin=$wifidaemon_dir/wifidaemon.sh
...
check_last_scan() {
    tempfile_path=$wifidaemon_dir/tmp/wifilist

    if [ ! -e "$tempfile_path" ]; then
        return 1
    fi

    last_scan=$(stat -c %Y "$tempfile_path")
    time=$(date +%s)

    if (( $time - $last_scan > 20 )); then
        return 1
    fi
...
wifimenu_setup() {
    if [ "$toggle" == "$tag_wifi_disable" ]; then
        current_wifi=$(cat $wifidaemon_dir/tmp/currentwifi)
        wifi_list=$(cat $wifidaemon_dir/tmp/wifilist)
...
show_manage_submenu() {
    ...
    known_network=$(echo -e "$tag_back\n$known_networks" | uniq -u | rofi_menu "$tag_manage_known_prompt" "$rows")
